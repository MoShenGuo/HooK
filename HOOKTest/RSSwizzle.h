...
/// A macro for wrapping the return type of the swizzled method.用于包装swizzled方法返回类型的宏。
#define RSSWReturnType(type) type

/// A macro for wrapping arguments of the swizzled method.用于包装swizzled方法的参数的宏。
#define RSSWArguments(arguments...) _RSSWArguments(arguments)

/// A macro for wrapping the replacement code for the swizzled method. 一个宏，用于包装swizzled方法的替换代码。
#define RSSWReplacement(code...) code

/// A macro for casting and calling original implementation.
/// May be used only in RSSwizzleInstanceMethod or RSSwizzleClassMethod macros.用于转换和调用原始实现的宏。
///只能在RSSwizzleInstanceMethod或RSSwizzleClassMethod宏中使用。
...
/**
 ...
 The selector is not available as a parameter to this block.实现方法的块。
 它的签名应该是:' method_return_type ^(id self, method_args…)'。
 选择器不能作为此块的参数。
 */
typedef id (^RSSwizzleImpFactoryBlock)(RSSwizzleInfo *swizzleInfo);

typedef NS_ENUM(NSUInteger, RSSwizzleMode) {
    /// RSSwizzle always does swizzling.RSSwizzle总是做swizzling。
    RSSwizzleModeAlways = 0,
    /// RSSwizzle does not do swizzling if the same class has been swizzled earlier with the same key.如果相同的类之前已经使用相同的键进行了swizzle，则RSSwizzle不会进行swizzling。
    RSSwizzleModeOncePerClass = 1,
    /// RSSwizzle does not do swizzling if the same class or one of its superclasses have been swizzled earlier with the same key.如果相同的类或它的一个超类之前已经使用相同的键进行了swizzle的swizzle，则RSSwizzle不会进行swizzling。
...
// and remove it later.为了防止没有参数时出现逗号问题，我们添加一个伪参数
//稍后再删除它。
